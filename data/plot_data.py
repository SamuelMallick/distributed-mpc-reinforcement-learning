...
with open("data/us_2_lr_4en5_fac_0_9995_exp_0.5_fac_0_99_stren_0_1_exprep_100_15_10/centralised.pkl", "rb") as file:
    X = pickle.load(file)
    U = pickle.load(file)
    R = pickle.load(file)
    TD = pickle.load(file)
...
axs[0, 1].plot(bounds[idx][: (int(limit / update_rate)),[0,2]], color= 'black', linewidth=0.6)
axs[1, 0].plot(f[idx][: (int(limit / update_rate))], color= 'black', linewidth=0.6)
axs[1, 1].plot(V0[idx].squeeze()[: (int(limit / update_rate))], color= 'black', linewidth=0.6)
